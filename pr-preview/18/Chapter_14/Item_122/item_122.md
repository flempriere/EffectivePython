...
- [Notes](#notes)
  - [Reordering Imports](#reordering-imports)
  - [Import, Configure, Run](#import-configure-run)
...
- A circular dependency results in one module being invoked by another
  ...
  - But the module can be loaded after the module is in `sys.modules`
    (Step 4.)
...
- Above, `app` imports `dialog`
  ...
  - `dialog` then imports `app`
    ...
    - `AttributeError` exception is raised since `prefs` is yet to be
      defined when accessed by `dialog`

- In general in these cases code should be refactored to avoid circular
  dependencies

  - Here we want `prefs` to be at the bottom of the dependency tree
    - Enables both `app` and `dialog` to import it as long as they are
...
- This solution is brittle
  - Latter changes might move the relative position of the `import` and
    break again
...
- But, should try to avoid this approach where possible
  ...
  - `import` statement has a performance cost
    - Can add up if the `import` statement is invoked frequently (See
      [Item 98](../../Chapter_11/Item_098/item_098.qmd))
...
- Circular dependencies arise when two modules call each other at
  ...
  - Can cause program crashes at start-up due to improper module
    initialisation
- Try to break circular dependencies by refactoring mutual dependencies
  into distinct modules at the bottom of a dependency tree
