...
- For example, we might have a timer program that occasionally needs to
  be interrupted and reset
...
- Calling `throw` on the generator with a `Reset` exception restarts the
  counter via the `except` block
