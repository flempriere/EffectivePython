...
- Python has only two visibility settings for attributes
  - *Public* or *Private* (more accurately name-mangled and denoted by a
    leading double underscore `__`)
...
```
    ...
    AttributeError: 'ChildClass' object has no attribute '_ChildClass__private_field'
    ---------------------------------------------------------------------------
    AttributeError                            Traceback (most recent call last)
    Cell In[2], line 12
          8         return self.__private_field
         11 baz = ChildClass()
    ---> 12 baz.get_private_field()

    Cell In[2], line 8, in ChildClass.get_private_field(self)
...
- Using private variables to lock down a class via `__` prevents
  ...
  - Subclassers can (and will) still access the private attributes, but
    the implementation becomes verbose and brittle
...
```
    ...
    AttributeError                            Traceback (most recent call last)
    Cell In[5], line 27
         23         return int(self._MyStringClass__value)  # Not updated and breaks
         26 bar = MyIntegerSubClass(5)
    ---> 27 assert bar.get_value() == 5

    Cell In[5], line 23, in MyIntegerSubClass.get_value(self)
         22 def get_value(self):
    ---> 23     return int(self._MyStringClass__value)
...
- Can reduce the chance of a clash with private attributes to force name
  mangling
...
- Support subclassing against internal API’s and attributes rather than
