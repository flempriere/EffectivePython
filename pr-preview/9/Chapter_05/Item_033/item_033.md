...
- Why does this function work?
  ...
  1.  Python supports *closures*
      - Functions referring to variables from the scope in which they
        are defined
      - i.e. above `helper` can access the `group` variable because both
        are in the `sort_priority` function’s scope
  2.  Python treats functions as *first-class* objects
      - You can refer to functions directly, assign them, pass them as
        arguments etc.
...
- Rather than use complicated `nonlocal` bindings prefer wrapping state
  in a helper class
...
- `nonlocal` let’s a function connect to and assign to variables in the
  enclosing scope
