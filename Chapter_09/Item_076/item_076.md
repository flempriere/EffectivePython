...
- [Notes](#notes)
  ...
  - [Porting to `async`](#porting-to-async)
...
- Now need to describe our `ClientSession` class
  ...
  - Guesses are requested from the server via the `request_number`
    method
    - Uses the provided `send` and `receive` references to communicate
      with the server
...
- We then have `run_client` on the main thread
  ...
  - It has been written using a mix of syntax
    ...
    - This is not for good style, but to demonstrate how to convert each
      style to coroutines
...
def run_client(address):
    time.sleep(0.1) # sleep to ensure time for the server to be set-up
...
```
    ...
    File ~/.local/share/uv/python/cpython-3.14.3-linux-x86_64-gnu/lib/python3.14/socket.py:870, in create_connection(address, timeout, source_address, all_errors)
        868 try:
    ...
    File ~/.local/share/uv/python/cpython-3.14.3-linux-x86_64-gnu/lib/python3.14/socket.py:855, in create_connection(address, timeout, source_address, all_errors)
...
- We’ll again use a stateful class to maintain the server state, but we
  need to update this to be `async`
  ...
  - `loop` needs to become `async` to handle the asynchronous methods
    ...
    - We `await` the `async` call to `send_numbers` which is the server
      sending a response
      - This just involves making `send_numbers` `async` and using an
        `await` call to `self.send`
...
- We then need to update our `new_game` function to handle async
  ...
  - However, we have to update the context manager to a special version
    that can handle `async`
...
- The glue code we wrote all worked under the model of using threads to
  handle the server and connections
  - Means we have to completely rewrite to the new paradigm (`async` +
    `asyncio` built-in)
...
- `run_async_server` takes an address as before
  ...
  - Instead use `asyncio` to `start_server` on the provided address via
    the `handle_async_connection` function
...
- The `run_client` function is almost entirely rewritten to support an
  ...
  - the initial socket connection code is replaced with an asynchronous
    equivalent
    - `asyncio.open_connection`
      - This returns a reader and writer stream rather than a connection
        socket
    - We use this to create a client
  - Then have to replace most of the body of the code for the various
    methods of running a game session with their `async` equivalents
  ...
  - We then have to do some different clean-up at the end
    ...
    - We have to close the `writer` then wait for it to be closed via
      `writer.wait_closed`
...
- For more difficult use cases `asyncio` offers a broader selection of
  features including (see [Item 77](../Item_077/item_077.qmd) and [Item
  78](../Item_078/item_078.qmd)),
  1. I/O
  2. Synchronisation
  3. Task Management
...
- Python provides asynchronous equivalents of standard language
  constructs
  - e.g. `for`, `with`, generators, comprehensions, iterators, library
    helper functions
