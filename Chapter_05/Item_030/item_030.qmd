...
- Python doesn't support pointer types (See [Item 95](../../Chapter_11/Item_095/item_095.qmd))
...
- Can't replace the reference with an entirely new object, but can mutate it
...
- Here we demonstrate doing so via slice operators (See [Item 14](../../Chapter_02/Item_014/item_014.qmd))
...
- User-defined classes are also modifiable (See [Item 29](../../Chapter_04/Item_029/item_029.qmd))
    - Any internal properties that can be accessed can be modified (See [Item 55](../../Chapter_07/Item_055/item_055.qmd))
...
- When implementing a function to be used by others,
    - don't modify mutable values except where documented explicitly in the function interface (in order of priority):
        1. The function name (make clear what's being modified)
        2. Function argument names (make clear which argument is being modified)
        3. Documentation (Provide detailed explanation)
- You should also consider creating a defensive copy of received arguments to prevent confusion
    - Especially when iteration is involved (see [Item 21](../../Chapter_03/Item_021/item_021.qmd), [Item 22](../../Chapter_03/Item_022/item_022.qmd))
- When calling a function,
    ...
    - For complex objects you control,
        ...
        - Or consider a functional approach ([Item 56](../../Chapter_07/Item_056/item_056.qmd))
...
- Functions should make it clear via naming and documentation when input arguments may be modified
    - In general, they should try to avoid the need to directly modify arguments
- Creating copies of collections and objects received as input ensures functions avoid accidentally modifying data
