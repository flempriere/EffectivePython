---
title: "Item 9: Consider `match` for Destructuring in Flow Control; Avoid When `if` Statements are Sufficient"
---

## Notes

- `match` was introduced in Python 3.10
- `match` has it's own mini-language embedded in python (See [Item 40](../../Chapter_06/Item_040/item_040.qmd) and [Item 44](../../Chapter_06/Item_044/item_044.qmd))
- `match` appears to  provide `switch`-like capabilities (See [Item 8](../Item_008/item_008.qmd))
...
- `match` is best thought of as analogous to variable unpacking (See [item 5](../Item_005/item_005.qmd))
...
- multiple assignment and tuple unpacking is an example of destructuring (See [Item 17](../../Chapter_03/Item_017/item_017.qmd)), e.g.
...
- We've seen that unpacking for lists and tuples can be used on deeply nested constructs too (See [Item 16](../../Chapter_02/Item_016/item_016.qmd))
...
- Code structure is regularised (See [Item 76](../../Chapter_09/Item_076/item_076.qmd))
...
- Now suppose we want to use class structure to capture our nodes (See [Item 29](../../Chapter_04/Item_029/item_029.qmd))
...
- Pretty similar to the original
    - We trade the namespace resolution of `tree.` for not having to do the tuple unpacking
...
- `match` also useful when serialisation of data and interpretation are decoupled
- e.g. Deserializing a JSON object (See [Item 54](../../Chapter_07/Item_054/item_054.qmd))
    - The Deserialized object is a nested structure of
        1. Dictionaries
        2. Lists
        3. Strings
        4. Numbers
- Responsibilities are not clearly encapsulated like with a class hierarchy (See [Item 53](../../Chapter_07/Item_053/item_053.qmd))
- The nesting structure gives the semantic meaning for a program
- For example, billing software looking to deserialize JSON customer records
...
- Naturally we want to convert these into proper python objects
    - Easier to integrate into data processes (See [Item 51](../../Chapter_07/Item_051/item_051.qmd))
    - UI widgets
    - etc.
...
- `match` supports a bunch of other forms of matching
    1. set patterns
    2. `as` patterns
    3. Positional constructor patterns (with `__match_args__` customisation)
    4. Exhaustiveness checking with type annotations (See [Item 124](../../Chapter_14/Item_124/item_124.qmd))
    5. and more...
- Refer to the official [tutorial](https://peps.python.org/pep-0636/)

## Things to Remember

- `match` statements can replace simple `if` statements but doing so is error prone
    - Structural nature of capture patterns in `case` clauses is unintuitive
- `match` statements give an way to combine `isinstance` checks and destructuring to manage control flow
    - Useful for processing heterogeneous data
    - Interpreting semantic meaning of semi-structured data
- `case`patterns can be used with built-in data structures and user-defined classes
