...
- `match` has it’s own mini-language embedded in python (See [Item
  40](../../Chapter_06/Item_040/item_040.qmd) and [Item
  44](../../Chapter_06/Item_044/item_044.qmd))
- `match` appears to provide `switch`-like capabilities (See [Item
...
- `match` is best thought of as analogous to variable unpacking (See
  [item 5](../Item_005/item_005.qmd))
...
- *Destructuring* means extracting components from a complex nested data
  structure
- multiple assignment and tuple unpacking is an example of destructuring
  (See [Item 17](../../Chapter_03/Item_017/item_017.qmd)), e.g.
...
- We’ve seen that unpacking for lists and tuples can be used on deeply
  nested constructs too (See [Item
...
- Code structure is regularised (See [Item
...
- Now suppose we want to use class structure to capture our nodes (See
  [Item 29](../../Chapter_04/Item_029/item_029.qmd))
...
- `match` also useful when serialisation of data and interpretation are
  decoupled
- e.g. Deserializing a JSON object (See [Item
...
- Responsibilities are not clearly encapsulated like with a class
  hierarchy (See [Item 53](../../Chapter_07/Item_053/item_053.qmd))
...
- Naturally we want to convert these into proper python objects
  - Easier to integrate into data processes (See [Item
...
- `match` supports a bunch of other forms of matching
  ...
  3. Positional constructor patterns (with `__match_args__`
      customisation)
  4. Exhaustiveness checking with type annotations (See [Item
