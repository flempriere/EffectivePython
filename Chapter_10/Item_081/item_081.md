...
```
    ...
    Cell In[1], line 5
          2 assert list_a, "a empty"  # Does not raise
          4 list_b = []
    ----> 5 assert list_b, "b empty"  # Raises
...
- Exceptions from `assert` are considered part of the internal
  ...
  - Self documenting
    - Second expression after the comma is evaluated to create a
      debugging method
  - Can combine with higher level tools for debug analysis (See [Item
    87](../Item_087/item_087.qmd)) e.g. reporting and logging
...
```
    ...
    ---------------------------------------------------------------------------
    RatingError                               Traceback (most recent call last)
    Cell In[4], line 20
         18 movie = Rating(5)
         19 movie.rate(5)  # Succeeds
    ---> 20 movie.rate(7)  # Raises

    Cell In[4], line 14, in Rating.rate(self, rating)
         12 def rate(self, rating):
         13     if not (0 < rating <= self.max_rating):
    ---> 14         raise RatingError("Invalid rating")
         15     self.ratings.append(rating)

    RatingError: Invalid rating

- This code uses `raise`
  - Indicates the caller should be caught and reported back to the end
    user or API caller
- If we instead take the view that the caller should not know about
  these errors we could instead write the class as
  - Here we’re assuming that other parts of the codebase have already
    validated `max_rating` and `rating`
...
```
    ...
    AssertionError: Invalid rating=5
    ---------------------------------------------------------------------------
    AssertionError                            Traceback (most recent call last)
    Cell In[5], line 13
          9         self.ratings.append(rating)
         12 movie = RatingInternal(5)
    ---> 13 movie.rate(5)
         14 movie.rate(7)  # Raises

    Cell In[5], line 8, in RatingInternal.rate(self, rating)
          7 def rate(self, rating):
    ----> 8     assert not (0 < rating <= self.max_rating), f"Invalid {rating=}"
          9     self.ratings.append(rating)

    AssertionError: Invalid rating=5

- An `assert` raising an exception is an indicator for a bug in the code
  - The reported message should assist in trying to identify and solve
