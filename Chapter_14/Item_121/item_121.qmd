...
- If we instead have a bug in the API itself it will bypass the insulation layer and propagate up
    ...
    - Caller needs to extend the `try/except` block that catches the broader Python `Exception` class to handle the error (See [Item 85](../../Chapter_10/Item_085/item_085.qmd))
